...
test.describe("Order Confirmation", () => {
  ...
  test("shows order confirmation with correct details", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /order confirmed|thank you/i }),
    ).toBeVisible()
  ...
  test("shows confirmation email notice", async ({ page }) => {
    await page.goto("{{baseUrl}}/order-confirmation/{{orderId}}")
    await expect(
      page.getByText(/confirmation.*sent to|email.*{{username}}/i),
    ).toBeVisible()
  })
  ...
  test('"view your orders" link navigates to order history', async ({
    page,
  }) => {
    await page.goto("{{baseUrl}}/order-confirmation/{{orderId}}")
