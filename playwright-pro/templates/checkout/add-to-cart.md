...
test.describe("Add to Cart", () => {
  ...
  test("add to cart button disabled for out-of-stock product", async ({
    page,
  }) => {
  ...
  test("shows error when quantity exceeds available stock", async ({
    page,
  }) => {
    await page
      .getByRole("spinbutton", { name: /quantity/i })
      .fill("{{overStockQuantity}}")
    await page.getByRole("button", { name: /add to cart/i }).click()
    await expect(page.getByRole("alert")).toContainText(
      /only.*available|exceeds.*stock/i,
    )
  })
