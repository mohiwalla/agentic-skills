...
test.describe("Update Cart Quantity", () => {
  ...
  test("increases item quantity", async ({ page }) => {
    ...
    await expect(
      row.getByRole("spinbutton", { name: /quantity/i }),
    ).toHaveValue("3")
    await expect(
      page.getByRole("region", { name: /order summary/i }),
    ).toContainText("{{updatedTotal}}")
  })
  ...
  test("decrease button disabled at minimum quantity", async ({ page }) => {
    ...
    await qty.press("Tab")
    await expect(
      row.getByRole("button", { name: /decrease|minus/i }),
    ).toBeDisabled()
  })
