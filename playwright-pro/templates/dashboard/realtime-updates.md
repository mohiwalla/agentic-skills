...
test.describe("Realtime Updates", () => {
  ...
  test('shows "connected" status indicator', async ({ page }) => {
    await page.goto("{{baseUrl}}/dashboard")
    await expect(
      page.getByRole("status", { name: /live|connected/i }),
    ).toBeVisible()
  })
  ...
  test("highlights updated value briefly", async ({ page }) => {
    ...
    // Highlight fades
    await expect(page.getByTestId("{{metricId}}")).not.toHaveClass(
      /updated|flash/,
    )
  })
  ...
  test("shows disconnected state when WebSocket closes", async ({ page }) => {
    ...
    await expect(
      page.getByRole("status", { name: /disconnected|offline/i }),
    ).toBeVisible()
    await expect(page.getByText(/reconnecting/i)).toBeVisible()
  })

  // Error case: connection refused — error state shown
  test("shows connection error when WebSocket cannot connect", async ({
    page,
  }) => {
  ...
  test("reconnects automatically after network interruption", async ({
    page,
  }) => {
    await page.goto("{{baseUrl}}/dashboard")
    await page.evaluate(() => {
    ...
    await expect(
      page.getByRole("status", { name: /disconnected/i }),
    ).toBeVisible()
    // Wait for auto-reconnect
    await expect(
      page.getByRole("status", { name: /connected|live/i }),
    ).toBeVisible()
  })
