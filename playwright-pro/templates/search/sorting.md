...
test.describe("Search Sorting", () => {
  ...
  test("sorts results by newest date first", async ({ page }) => {
    await page
      .getByRole("combobox", { name: /sort by/i })
      .selectOption("date_desc")
    await expect(page).toHaveURL(/sort=date_desc/)
    const dates = page.getByTestId("result-date")
    const firstDate = new Date(
      (await dates.first().getAttribute("datetime")) ?? "",
    )
    const secondDate = new Date(
      (await dates.nth(1).getAttribute("datetime")) ?? "",
    )
  ...
  test("sort selection persists when filter applied", async ({ page }) => {
    ...
    await expect(page.getByRole("combobox", { name: /sort by/i })).toHaveValue(
      "price_asc",
    )
  })
...
test.describe("Search Sorting", () => {
  ...
  test("sorts by price low to high", async ({ page }) => {
    ...
    const prices = page.getByTestId("result-price")
    const a = parseFloat(
      (await prices.first().textContent()).replace(/[^0-9.]/g, ""),
    )
    const b = parseFloat(
      (await prices.nth(1).textContent()).replace(/[^0-9.]/g, ""),
    )
