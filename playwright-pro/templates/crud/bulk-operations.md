...
test.describe("Bulk Operations", () => {
  ...
  test("shows bulk action toolbar only when items are selected", async ({
    page,
  }) => {
    await expect(
  ...
  test('shows "select all across pages" option when applicable', async ({
    page,
  }) => {
    await page.getByRole("checkbox", { name: /select all/i }).check()
    const crossPage = page.getByRole("button", {
      name: /select all.*across pages/i,
    })
