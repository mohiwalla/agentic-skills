...
test.describe("Email Verification", () => {
  ...
  test("verifies email with valid token", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /email verified|verified/i }),
    ).toBeVisible()
    await expect(
      page.getByRole("link", { name: /continue|go to dashboard/i }),
    ).toBeVisible()
  })
  ...
  test("shows error for expired verification token", async ({ page }) => {
    await page.goto(
      "{{baseUrl}}/verify-email?token={{expiredVerificationToken}}",
    )
    await expect(
      page.getByRole("heading", {
        name: /link.*expired|verification.*failed/i,
      }),
    ).toBeVisible()
    await expect(
      page.getByRole("link", { name: /resend|request new/i }),
    ).toBeVisible()
  })
  ...
  test("shows error for invalid verification token", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /invalid|failed/i }),
    ).toBeVisible()
  })
  ...
  test("shows already verified message for used token", async ({ page }) => {
    ...
    await expect(
      page.getByText(/already verified|email.*confirmed/i),
    ).toBeVisible()
...
test.describe("Email Verification", () => {
  ...
  test("shows verification prompt on login for unverified user", async ({
    page,
  }) => {
    await page.goto("{{baseUrl}}/login")
    await page
