...
test.describe("First-Time Setup", () => {
  ...
  test("shows setup wizard on first login", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /set up.*account|get started/i }),
    ).toBeVisible()
  })
  ...
  test("completes organisation details step", async ({ page }) => {
    ...
    await page
      .getByRole("textbox", { name: /organisation.*name|company/i })
      .fill("{{orgName}}")
    await page
      .getByRole("combobox", { name: /industry/i })
      .selectOption("{{industry}}")
    await page
      .getByRole("spinbutton", { name: /team size/i })
      .fill("{{teamSize}}")
  ...
  test("completes preferences step", async ({ page }) => {
    ...
    await page
      .getByRole("combobox", { name: /timezone/i })
  ...
  test("shows validation when required field missing", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /next/i }).click()
    await expect(
      page.getByText(/organisation.*required|required/i),
    ).toBeVisible()
  })
