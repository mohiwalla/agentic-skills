...
test.describe("Single-Step Form — {{formName}}", () => {
  ...
  test("submits form with valid data", async ({ page }) => {
    await page
      .getByRole("textbox", { name: /{{field1Label}}/i })
      .fill("{{field1Value}}")
    await page
      .getByRole("textbox", { name: /{{field2Label}}/i })
      .fill("{{field2Value}}")
    await page
      .getByRole("combobox", { name: /{{field3Label}}/i })
      .selectOption("{{field3Value}}")
    await page.getByRole("button", { name: /submit|save/i }).click()
    await expect(page.getByRole("alert")).toContainText(
      /submitted|saved successfully/i,
    )
  })
  ...
  test("reset button clears all fields", async ({ page }) => {
    ...
    await expect(
      page.getByRole("textbox", { name: /{{field1Label}}/i }),
    ).toHaveValue("")
  })
  ...
  test("shows required field error", async ({ page }) => {
    await page.getByRole("button", { name: /submit|save/i }).click()
    await expect(
      page.getByText(/{{field1Label}}.*required|required/i),
    ).toBeVisible()
    await expect(
      page.getByRole("textbox", { name: /{{field1Label}}/i }),
    ).toBeFocused()
...
test.describe("Single-Step Form — {{formName}}", () => {
  ...
  test("disables submit after click (prevents double submit)", async ({
    page,
  }) => {
    await page
      .getByRole("textbox", { name: /{{field1Label}}/i })
