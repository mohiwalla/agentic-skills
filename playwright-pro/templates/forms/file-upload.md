...
test.describe("File Upload", () => {
  ...
  test("uploads a single file successfully", async ({ page }) => {
    await page
      .getByRole("button", { name: /choose file|browse/i })
      .setInputFiles(testFile)
    await expect(page.getByText(/{{testFileName}}/)).toBeVisible()
    await page.getByRole("button", { name: /upload/i }).click()
    await expect(page.getByRole("progressbar")).toBeVisible()
    await expect(page.getByRole("alert")).toContainText(
      /upload.*complete|uploaded successfully/i,
    )
  })
  ...
  test("removes file from queue", async ({ page }) => {
    await page.locator('input[type="file"]').setInputFiles(testFile)
    await page
      .getByRole("button", { name: /remove.*{{testFileName}}|×/i })
      .click()
  ...
  test("shows error for oversized file", async ({ page }) => {
    ...
    await expect(
      page.getByText(/too large|exceeds.*{{maxFileSizeMb}}|max.*size/i),
    ).toBeVisible()
  })
  ...
  test("shows error for unsupported file type", async ({ page }) => {
    ...
    await expect(
      page.getByText(/unsupported.*type|{{acceptedMimeTypes}}.*only/i),
    ).toBeVisible()
  })
