...
async function completeStep2(page: Page): Promise<void> {
  await page
    .getByRole("combobox", { name: /{{step2Field}}/i })
    .selectOption("{{step2Value}}")
  await page
    .getByRole("textbox", { name: /{{step2TextField}}/i })
    .fill("{{step2TextValue}}")
  await page.getByRole("button", { name: /next/i }).click()
}
...
test.describe("Multi-Step Form", () => {
  ...
  test("navigates back to previous step without losing data", async ({
    page,
  }) => {
    await completeStep1(page)
  ...
  test("clicking completed step in indicator navigates back", async ({
    page,
  }) => {
    await completeStep1(page)
  ...
  test("browser back from step 2 returns to step 1 with data", async ({
    page,
  }) => {
    await completeStep1(page)
    await page.goBack()
