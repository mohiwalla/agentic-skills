...
test.describe("REST CRUD — /{{entityName}}s", () => {
  ...
  test("GET /{{entityName}}s/:id returns entity", async ({ request }) => {
    const res = await request.get(
      `{{apiBaseUrl}}/{{entityName}}s/{{existingEntityId}}`,
      { headers },
    )
    expect(res.status()).toBe(200)
    const body = await res.json()
  ...
  test("PATCH /{{entityName}}s/:id partially updates entity", async ({
    request,
  }) => {
  ...
  test("DELETE /{{entityName}}s/:id deletes entity", async ({ request }) => {
    const del = await request.delete(
      `{{apiBaseUrl}}/{{entityName}}s/{{deletableEntityId}}`,
      { headers },
    )
  ...
  test("GET non-existent entity returns 404", async ({ request }) => {
    const res = await request.get("{{apiBaseUrl}}/{{entityName}}s/999999", {
      headers,
    })
