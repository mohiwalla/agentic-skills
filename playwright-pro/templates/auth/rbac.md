...
test.describe("RBAC — Regular User", () => {
  ...
  test("regular user sees 403 on admin panel", async ({ page }) => {
    ...
    const forbidden = page.getByRole("heading", {
      name: /403|forbidden|not authorized/i,
    })
  ...
  test("regular user cannot delete another user's resource", async ({
    page,
  }) => {
...
test.describe("RBAC — Role Elevation", () => {
  ...
  test("newly promoted admin can access admin panel", async ({ browser }) => {
    ...
    await expect(
      userPage.getByRole("heading", { name: /admin/i }),
    ).toBeVisible()
    await userCtx.close()
  })
})
