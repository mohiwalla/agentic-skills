...
test.describe("Notification Preferences", () => {
  ...
  test("changes notification frequency", async ({ page }) => {
    await page
      .getByRole("combobox", { name: /frequency|digest/i })
      .selectOption("{{frequency}}")
    await page.getByRole("button", { name: /save/i }).click()
    await expect(page.getByRole("alert")).toContainText(/saved/i)
    await page.reload()
    await expect(
      page.getByRole("combobox", { name: /frequency|digest/i }),
    ).toHaveValue("{{frequency}}")
  })
  ...
  test("shows confirmation before unsubscribing all", async ({ page }) => {
    ...
    await expect(
      page.getByRole("dialog", { name: /unsubscribe/i }),
    ).toBeVisible()
    await page.getByRole("button", { name: /cancel/i }).click()
    // Still subscribed
    await expect(
      page.getByRole("switch", { name: /email notifications/i }),
    ).toBeChecked()
  })
})
```
