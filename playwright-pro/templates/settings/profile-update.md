...
test.describe("Profile Update", () => {
  ...
  test("updates display name", async ({ page }) => {
    const nameField = page.getByRole("textbox", {
      name: /display name|full name/i,
    })
    await nameField.clear()
    await nameField.fill("{{newName}}")
    await page.getByRole("button", { name: /save|update/i }).click()
    await expect(page.getByRole("alert")).toContainText(
      /profile updated|saved/i,
    )
    await expect(
      page.getByRole("textbox", { name: /display name|full name/i }),
    ).toHaveValue("{{newName}}")
  })
  ...
  test("updates email address", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /save|update/i }).click()
    await expect(page.getByRole("alert")).toContainText(
      /verification.*sent|email updated/i,
    )
  })
  ...
  test("uploads new avatar image", async ({ page }) => {
    await page
      .getByRole("button", { name: /change.*avatar|upload.*photo/i })
      .click()
  ...
  test("nav shows updated name after save", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /save|update/i }).click()
    await expect(
      page.getByRole("navigation").getByText("{{newName}}"),
    ).toBeVisible()
  })
})
```
...
test.describe("Profile Update", () => {
  ...
  test("updates display name", async ({ page }) => {
    ...
    await page
      .getByRole("textbox", { name: /display name|full name/i })
      .fill("{{newName}}")
    await page.getByRole("button", { name: /save|update/i }).click()
    await expect(page.getByRole("alert")).toContainText(
      /profile updated|saved/i,
