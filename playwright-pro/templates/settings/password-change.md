...
test.describe("Password Change", () => {
  ...
  test("changes password with valid inputs", async ({ page }) => {
    ...
    await expect(page.getByRole("alert")).toContainText(
      /password.*changed|updated successfully/i,
    )
  })
  ...
  test("shows strength error for weak new password", async ({ page }) => {
    ...
    await expect(
      page.getByText(/too weak|at least \d+ characters/i),
    ).toBeVisible()
  })
  ...
  test("shows error when new password matches current", async ({ page }) => {
    ...
    await page.getByRole("button", { name: /change.*password/i }).click()
    await expect(
      page.getByText(/same as.*current|choose.*different/i),
    ).toBeVisible()
  })
  ...
  test("strength meter reacts to new password input", async ({ page }) => {
    ...
    await expect(
      page.getByRole("meter", { name: /strength/i }),
    ).toHaveAttribute("aria-valuenow", "1")
    await page
      .getByRole("textbox", { name: /^new password$/i })
      .fill("Str0ng!Pass#2026")
    await expect(
      page.getByRole("meter", { name: /strength/i }),
