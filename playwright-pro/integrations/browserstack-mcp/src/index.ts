	...
	tools: [
		{
			...
			description:
				"Get BrowserStack Automate plan details including parallel session limits",
			inputSchema: { type: "object", properties: {} },
		},
		{
			name: "browserstack_get_browsers",
			description:
				"List all available browser and OS combinations for Playwright testing",
			inputSchema: { type: "object", properties: {} },
		},
		{
		...
		},
		{
			...
			description:
				"Get detailed session info including video URL, logs, and screenshots",
			inputSchema: {
				type: "object",
				properties: {
					session_id: {
						type: "string",
						description: "Session hashed ID",
					},
				},
				required: ["session_id"],
			},
		},
		{
			...
			description:
				"Update session status (mark as passed/failed) and name",
			inputSchema: {
				type: "object",
				properties: {
					session_id: {
	...
	try {
		switch (name) {
			...
			case "browserstack_get_builds": {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(builds, null, 2) },
					],
				}
			}
			...
			case "browserstack_update_session": {
				...
				if (args?.status)
					update.status = args.status as "passed" | "failed"
			...
			case "browserstack_get_logs": {
				const logs = await client.getSessionLogs(
					args!.session_id as string,
				)
