		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					section_id: {
						type: "number",
						description: "Section ID to add the case to",
					},
					title: { type: "string", description: "Test case title" },
					template_id: {
						type: "number",
						description: "Template ID (2 = Test Case Steps)",
					},
					priority_id: {
						type: "number",
						description:
					...
					custom_preconds: {
						type: "string",
						description: "Preconditions text",
					},
		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					...
					include_all: {
						type: "boolean",
						description: "Include all cases (default true)",
					},
					case_ids: {
						type: "array",
						items: { type: "number" },
		...
		},
		{
			...
			inputSchema: {
				...
				properties: {
					...
					comment: {
						type: "string",
						description: "Result comment or error message",
					},
					elapsed: {
						type: "string",
						description: 'Time spent (e.g., "30s", "1m 45s")',
					},
					defects: {
						type: "string",
						description: "Defect IDs (comma-separated)",
					},
				},
	...
	try {
		switch (name) {
			...
			case "testrail_get_suites": {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(suites, null, 2) },
					],
				}
			}
			...
			case "testrail_get_cases": {
				...
				return {
					content: [
						{ type: "text", text: JSON.stringify(cases, null, 2) },
					],
				}
			}
			...
			case "testrail_add_case": {
				const payload: TestRailCasePayload = {
					...
					custom_preconds: args?.custom_preconds as
						| string
						| undefined,
					custom_steps_separated:
						args?.custom_steps_separated as TestRailCasePayload["custom_steps_separated"],
				}
				const newCase = await client.addCase(
				...
				return {
					content: [
						{
							type: "text",
							text: JSON.stringify(newCase, null, 2),
						},
					],
				}
			}
			...
			case "testrail_update_case": {
				...
				if (args?.custom_preconds)
					updatePayload.custom_preconds =
						args.custom_preconds as string
